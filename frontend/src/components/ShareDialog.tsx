...
export function ShareDialog({ videoId, startTime, endTime, trigger }: ShareDialogProps) {
    ...
    <Dialog.Root open={open} onOpenChange={setOpen}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white p-6 shadow-lg">
          <Dialog.Title className="text-lg font-semibold mb-4">Create Share Link</Dialog.Title>

          <Dialog.Description className="text-sm text-gray-600 mb-4">
            Share this timestamp: {startTime.toFixed(1)}s{endTime && ` - ${endTime.toFixed(1)}s`}
          </Dialog.Description>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  <Input value={fullShareUrl} readOnly className="font-mono text-xs" />
